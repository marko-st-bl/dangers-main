        ...
        <div class="row">
            ...
            <div class="profile-sidebar col-sm-3 col-md-2 bg-light">
                <div class="avatar row justify-content-center">
                    <img class="rounded-circle" src="<%= userBean.getUser().getAvatar() %>" />
                </div>
            ...
            <div class="posts-container col-sm-9 col-md-8">
                ...
                <div class="post-form-container">
                    ...
                    <div class="post-form">
                        <form id="new-post-form" class="form" action="http://localhost:8080/dangers-main/NewPost" method="POST" enctype="multipart/form-data">
                            <div class="row justify-content-center">
                                <textarea id="post-text" class="form-control col-11" name="text" cols="30" rows="3"
                                    placeholder="Share text, image, video or link"></textarea>
                            </div>
                            <div class="row justify-content-center">
                                <div class="col-11">
                                    <button id="img-btn" class="btn"
                                        type="button"><i class="fas fa-image"></i></button>
                                    <button id="video-btn" class="btn"
                                        type="button"><i class="fas fa-video"></i></button>
                                        <button id="yt-btn" class="btn" type="button"><i class="fab fa-youtube"></i></button>
                                    <button id="link-btn" class="btn" type="button"><i class="fas fa-link"></i></button>
                                </div>
                            </div>
                            ...
                            <input type="text" id="post-type" name="type" value="text" style="display: none"/>
                            <div class="row justify-content-center">
                                <input id="link-input" class="form-control p-2 col-11" type="url" id="link-input" name="link" placeholder="Paste url" style="display: none"/>
                            </div>
                            <div class="row justify-content-center">
                                <input id="youtube-input" class="p-2 col-11 form-control" type="url" name="youtube" placeholder="Paste youtube link" style="display: none"/>
                            </div>
                            <div class="row justify-content-center">
                                <img id="img-preview" class="p-1 col-11" src="">
                                <video id="video" class="p-1 col-11" width="100%" controls style="display: none">
                ...
                <div class="posts">
                    <div class="post my-1">
                        <div class="row align-items-center">
                            <img class="avatar rounded-circle" src="https://restcountries.eu/data/srb.svg" alt="">
                            <h5 class="p-2">Marko Stojanovic</h5>
                        </div>
                        <div class="row">
                            <p class="my-1">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quasi vel quae
                                reiciendis consequuntur distinctio ullam itaque voluptatem quisquam illum porro fuga
                                debitis labore, voluptatibus suscipit ad culpa sequi officia est!</p>
                        </div>
                        <div class="row">
                            <img class="my-2 post-image" src="https://restcountries.eu/data/srb.svg" alt="">
                        </div>
                        <div class="row">
                            <button class="post-button btn col-4" type="button"><i class="far fa-comment"></i> Comment</button>
                        ...
                        <div id="share-buttons" class="flex" style="display: none">
                ...
                if (this.status == 200) {
                    ...
                    for (let i in countries) {
                        if (countries[i].name == country) {
                            ...
                            url = 'http://api.openweathermap.org/data/2.5/find?lat=' + lat + '&lon=' + lon + '&cnt=30&units=metric&&APPID=529b81310379224f19501c0caf39800b';
                            ...
                            xhr1.onload = function () {
                                if (this.status == 200) {
                                    ...
                                    document.getElementById('icon1').src = 'http://openweathermap.org/img/wn/' + forecasts.list[forecast1].weather[0].icon + '@2x.png';
                                    document.getElementById('temp1').innerHTML = Math.round(forecasts.list[forecast3].main.temp) + '&#8451;';
                                    document.getElementById('cityName2').innerHTML = forecasts.list[forecast2].name;
                                    document.getElementById('icon2').src = 'http://openweathermap.org/img/wn/' + forecasts.list[forecast2].weather[0].icon + '@2x.png';
                                    document.getElementById('temp2').innerHTML = Math.round(forecasts.list[forecast3].main.temp) + '&#8451;';
                                    document.getElementById('cityName3').innerHTML = forecasts.list[forecast3].name;
                                    document.getElementById('icon3').src = 'http://openweathermap.org/img/wn/' + forecasts.list[forecast3].weather[0].icon + '@2x.png';
                                    document.getElementById('temp3').innerHTML = Math.round(forecasts.list[forecast3].main.temp) + '&#8451;';
                                }
                            }
                            xhr1.send();
                        }
        	...
        	$.ajax({
        		...
        		for(let i in posts){
        			if (posts[i].type == "image"){
        				$('.posts').append(
        					'<div class="post my-1">' +
        						'<div class="row align-items-center">' +
        							...
        							'<h5 class="p-2">' + posts[i].owner.firstName + posts[i].owner.lastName + '</h5>' +
        						'</div>' +
        						'<div class="row text-muted">' +
									posts[i].createdAt +
								'</div>' +
        						'<div class="row">' +
        						'<p class="my-1">' + posts[i].text + '</p>' +
        						'</div>' +
        						'<div class="row">' +
        				...
        				$('.posts').append(
            					'<div class="post my-1">' +
            						...
            						'<div>' +
            						'<div class="row">' +
                            			'<img id="comment-img-preview-'+posts[i].id+'" src="" width="100%">' +
                        			'</div>' +
            					'</div>'
            				);
        				...
        				$('.posts').append(
        						'<div class="post my-1">' +
        							'<div class="row">' +
        								'<h5 class="pl-2">' + posts[i].title + '</h5>' + 
        							'</div>' +
        							'<div class="row text-muted">' +
        								posts[i].createdAt +
        							'</div>' +
        							'<div class="row">' +
        								'<p class="my-1">' + posts[i].text +
            					...
            					'<div class="post my-1">' +
            						...
            						'<div class="row">' +
            							'<div class="embed-responsive embed-responsive-16by9">' +
            								'<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/'+ posts[i].url.split('?v=')[1] +'" allowfullscreen></iframe>' +
            							'</div>' +
            						'</div>' +
            						'<div class="row">' +
            							'<button class="post-button btn col-4" type="button"><i class="far fa-comment"></i> Comment</button>' +
        			...
        			if(posts[i].type !== 'rss'){
        				...
        				$('#comment-btn-'+posts[i].id).click(function(){
        					$('#comment-container-'+posts[i].id).toggle();
        				});
        		            $('#new-comment-btn-'+ posts[i].id).click(function(){
        		                $('#comment-img-input-'+posts[i].id).trigger('click');
        		            });
        		            $('#comment-img-input-'+posts[i].id).change(function(){
        		                let file = this.files[0];
        		                if (file) {
        		                let src = URL.createObjectURL(file);
        		                console.log(src);
        		                $('#comment-img-preview-'+posts[i].id).attr('src', src);
        		            } else {
        		                $('#comment-img-preview-'+posts[i].id).attr('src', '');
        		            }
        		            });
        		            $('#comment-input-'+posts[i].id).keypress(function(e){
        		                if(e.which == 13){
        		                    e.preventDefault();
        		                    if($('#comment-input-'+posts[i].id).val()){
        		                        let form = $('#form-' + posts[i].id);
        		                        console.log(form);
        		                        form.submit();
        		                    }
        		                }
        		            });
        		            $('#form-'+posts[i].id).submit(function(e){
        		                e.preventDefault();
        		                $.ajax({
